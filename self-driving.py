"""auto_driver.p]y: Multi-threaded server program that receives jpeg video frames, ultrasonic
...
class NeuralNetwork():
    def __init__(self):
        self.model = cv2.ml.ANN_MLP_load('mlp_xml/4_25_OK_version2_mlp.xml')
...
class ObjectDetection(object):
    ...
    def detect(self, image, gray, classifier):
        ...
        # Camera coordinates for the y-axis pertaining to point on the target object, 
        v = 0
...
class VideoStreamHandler(socketserver.StreamRequestHandler):
    # Cascade Classifiers
    stop_classifier = cv2.CascadeClassifier('cascade_classifiers/stop_sign1.xml')
    ...
    def handle(self):
        ...
        try:
            while True:
                ...
                '''if (v_stop is not None) & (v_light is not None):
                ...
                if sensor_data is not None and sensor_data < 50:
                    ...
                    print('start',stop_start)
                    print('finish',stop_finish)
...
class ThreadServer():
    ...
    video_thread = threading.Thread(target=server_video_thread, args=('192.168.192.184', 8000))
    video_thread.start()

    ultrasonic_sensor_thread = threading.Thread(target=ultrasonic_server_thread('192.168.192.184', 8002))
